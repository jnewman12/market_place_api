...
describe Order do 
	...
	describe '#set_total!' do
	   ...
	   it "returns the total amount to pay for the products" do
	     ...
	     lambda { @order.set_total! }.should change{@order.total}.from(0).to(1575)
	   end
	end
	...
	describe "#build_placements_with_product_ids_and_quantities" do 
	  ...
	  it "builds 2 placements for the order" do
	    ...
	    lambda { order.build_placements_with_product_ids_and_quantities(@product_ids_and_quantities) }.should change{order.placements.size}.from(0).to(2) 
	  end
	end
